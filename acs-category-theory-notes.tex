...
\chapter{Functors}
\lecturedetails{24 October 2017}{M Fiore, J Clarke}

\section{Duality}
\begin{definition}[Duality]
Given a category $\mathcal{C}$, we define the \emph{opposite} or \emph{dual},
$\mathcal{C}^{\mathrm{op}}$ to be the category with:
    \begin{itemize}
...
\begin{equation*}
    ...
    \equalto{\mathcal{C}^{\mathrm{op}}(A, B)}{\mathcal{C}(B, A)} \xrightarrow{\comp^{\mathrm{op}}_{A,B,C}}
...
Here, $T$ can be thought of as a set of tests.
\end{definition}
...
retractions are closed under composition, \ie~if $f : A \to B$, $g : B \to C$
are in the class, then $g \comp f : A \to C$ is also in the class.
\end{exercise}
...
\begin{center}
    \begin{tikzpicture}
        \matrix[matrix of math nodes, name=m, column sep=0.6em, row sep=1.2em] {
            ...
            |[alias=Mono]| \mathcal{C}_{\mathrm{mono}} &
            \\
            |[alias=Cong]| \mathcal{C}_{\mathrm{\cong}} &
            |[alias=Ex1]| &
            |[alias=Ex2]| &
            |[alias=C]| \mathcal{C}
            \\
            ...
            |[alias=Epi]| \mathcal{C}_{\mathrm{epi}} &
            \\
        };
...
\begin{equation*}
    \left(\prod_i A_i\right) \xrightarrow{\pi_k} A_k
\end{equation*}

for all $k \in I$, such that this is a terminal universal solution.
...
In $\catset$, disjoint unions are coproducts:
