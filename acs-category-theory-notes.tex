...
\begin{remark}
    Recall the universal problem of generating a monoid $(S*, \cdot, \epsilon)$
    from a set $S$ with a function $\sigma$. Compare the definition of a
    universal solution to this problem with the definition of a bijection $ f :
    S \rightarrow T$:
    \begin{alignat*}{3}
        & \forall (M, f).\ &&
        \exists!\ f^\# : S* \rightarrow M.\ &&
        f = f^\# \comp \sigma \text{ and }
    ...
    The similarity suggests that solving a universal problem amounts to
    establishing a bijection between a potential solutions and morphisms that
    ``convert'' these potential solutions into the universal one. To see this,
    consider the hom-set of the generated monoid and any proposed solution $(M,
    \shortmid, \star)$
    \begin{equation*}
        H_{\catmon} = \catmon ( (S*, \cdot, \epsilon), (M, \shortmid, \star) )
    \end{equation*}
    and the hom-set of our source set $S$ and the underlying set $M$ of the
    proposed solution:
    \begin{equation*}
    ...
    \begin{tikzcd}
        ...
        & (S*, \cdot, \epsilon) \arrow[d, "\exists! f^\# \text{ homomorphism}"]
        \\
        {} & (M, \shortmid, \star)
    \end{tikzcd}
    \end{center}

    The universality condition states that for each function $f \in H_{\catset}$
    we must find a unique $f^\# \in H_{\catmon}$ such that $f = f^\# \comp
    \sigma$ and $f^\#$ is a homomorphism. That is, we must find a bijection
    between $H_{\catmon}$ and $H_{\catset}$:

    \begin{equation*}
        \catmon ( (S*, \cdot, \epsilon), (M, \shortmid, \star) ) \simeq
        \catset ( S, M )
    \end{equation*}
...
\begin{itemize}
    ...
    \item Morphisms: a pair of functions $(\eta, \epsilon)$, where $\eta : N
    \rightarrow N$ maps nodes to nodes and $\epsilon : E \rightarrow E$ maps
    ...
    there will be an edge $\epsilon(e)$ between nodes $\eta(a)$ and $\eta(b)$ in
    a graph that is connected to $G$ by a graph morphism $(\eta, \epsilon)$.
\end{itemize}
\begin{center}
\begin{tikzcd}[row sep=0.8em]
      {}
    & E_1 \arrow[ld, swap, "s_1"] \arrow[dd, "\epsilon"] \arrow[rd, "t_1"]
    & {} \\
...
\begin{tikzcd}[row sep=4em]
      ...
      & b \arrow[d, dashed, "\eta"] \\
        \eta(a) \arrow[r, "\epsilon(e)"{name=D}]
      & \eta(b)
      \arrow[from=U, to=D, dashed, "\epsilon"]
\end{tikzcd}
\end{center}
