...
\chapter{Functors}
\lecturedetails{24 October 2017}{M Fiore, T Parks}
...
\begin{center}
\begin{tikzcd}
    \mathcal{A} \arrow[r, "\mathcal F"]  & \mathcal{B}
\end{tikzcd}
\end{center}
...
\centering
\begin{tabular}{llll}
 Monoids                                                               & PreOrders                                                         & Catagories                                                                    \\
 $(M_1, e_1, \ast_1) \stackrel{h}{\longrightarrow} (M_2, e_2, \ast_2)$ & $(P_1, \leq_1) \stackrel{f}{\longrightarrow} (P_2, \leq_2)$       & $\cat{C}_1 \stackrel{\mathcal{F}}{\longrightarrow} \cat{C}_2 $                \\
 $M_1 \stackrel{h}{\longrightarrow} M_2$                               & $P_1 \stackrel{f}{\longrightarrow} P_2$                           & $\mathcal{F_{\textrm{objects}}} : \Ob(\cat{C}_1) \longrightarrow \Ob(\cat{C}_2)$  \\
                                                                       &                                                                   & $\mathcal{F_{\textrm{arrows}}} : \Arr(\cat{C}_1) \longrightarrow \Arr(\cat{C}_2)$ \\
 $h(e_1) = e_2$                                                        & $\forall p,q \in P_1 \ldotp p \leq_1 q \implies f(p) \leq_2 f(q)$ & $\mathcal{F_{\textrm{arrows}}}( \id{ \cat{C}_1 } ) = \id{ \mathcal{F_{\textrm{objects}}}({\mathbf{C}_1}) }$ \\
                                                                       &                                                                   &

% the yshift is needed to make the F_arrow level.
\begin{tikzcd}
P \arrow[dd, ""{name=L, right}] &  & F_{objects}(P) \arrow[dd, ""{name=R, left}] \\
\arrow[rr, "F_{arrows}", from=L, to=R, start anchor={[yshift=-0.2ex]},] &  &  \\
Q &  & F_{objects}(P)
\end{tikzcd} \\

 $ h(f \comp g) = h(f) \comp h(g) $                                  &                                                                    &

 \begin{tikzcd}
A \arrow[d, "f"] \arrow[dd, "g \comp f"', bend right=49] &  & F_{objects}(A) \arrow[d, "F_{arrow}(f)"] \arrow[dd, "F_{arrow}(g \comp f)", bend left=60] \\
B \arrow[d, "g"] & \arrow[r, equals] & F_{objects}(B) \arrow[d, "F_{arrow}(g)", dashed] \\
C &  & F_{objects}(C)
\end{tikzcd}
