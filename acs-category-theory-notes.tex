...
\newcommand{\fncid}[1]{\mathit{id}_{#1}}
...
\chapter{Functors continued}
\lecturedetails{26 October 2017}{M Fiore, N Alcock, Z Yuan}

\section{More about functors}

\begin{definition}
A functor \(F: \mathcal{A} \rightarrow \mathcal{B}\) specifies a function \(F: \Ob(\mathcal{A}) \rightarrow \Ob(\mathcal{B})\) such that \(\forall X, Y \in \Ob(\mathcal{A})\), there exists \(F_{X, Y}: \mathcal{A}(X, Y) \rightarrow \mathcal{B}(FX, FY)\) such that identities are preserved.
...
\begin{tikzcd}
X 
\arrow[dd, "f"{name=L, right}] & & FX 
\arrow[dd, "Ff"{name=R, left}] 
\\
\arrow[rr, "F_{XY}", from=L, to=R, start anchor={[yshift=-0.2ex]}, mapsto] 
&  &  
\\
Y &  & FY
\end{tikzcd}
...
\begin{center}
    \begin{tikzcd}
        \nelem{1} \arrow[r, "id^{(\mathcal{A})}_A"] \arrow[dr, swap, "id^{(\mathcal{B})}_B"] &
        \mathcal{A}\left(A,A\right) \arrow[d, "F_{AA}"]
        \\
        &
...
\begin{center}
    \begin{tikzcd}[ampersand replacement=\&]
        \mathcal{A}(Y,Z) \times  \mathcal{A}(X,Y) \arrow[r, "\circ_{X,Y,Z}^{(\mathcal{A})}"] \arrow [d, "F({Y,Z}) \times F({X,Y})"] \&
         \mathcal{A}(X,Z) \arrow[d, "F_{XZ}"] \&
           \\
        \mathcal{B}(FY,FZ) \times  \mathcal{B}(FX,FY) \arrow[r, "\circ_{FX,FY,FZ}^{(\mathcal{B})}"] \&
        \mathcal{B}(FX,FZ) \&
    \end{tikzcd} \\[3mm]
\end{center}

Compare with the monoid homomorphism, we have:

\begin{center}
    \begin{tikzcd}
        \nelem{1} \arrow[r, "l"] \arrow[dr, swap, "l'"] &
       M \arrow[d, "h"]
        \\
        &
         M'
...
\begin{center}
    \begin{tikzcd}[ampersand replacement=\&]
        M_1 \times M_1 \arrow[r, "\star_1"] \arrow [d, "h \times h"] \&
        M_1 \arrow[d, "h"] \&
           \\
...
For a given category $\smcat{C}$, $\fncid{\smcat{C}}: \smcat{C} \to \smcat{C}$ is a functor that maps objects to themselves and morphisms to themselves.

\begin{center}
\begin{tikzcd}
A \arrow[dd, "f" swap] & A \arrow[dd, "f"] \\
{} \arrow[r, mapsto, "\fncid{}(f)"] & {}\\
B & B
\end{tikzcd}
\end{center}

$\fnclist$ is a functor from \(\catset \to \catset\).

\begin{center}
\begin{tikzcd}
    S \arrow[r, "\eta^S"]  \arrow[rd, swap, "\forall\, f"]
  &
  \fnclist(S)
  \arrow[d, "f^\#"]
  & (\fnclist(S), nil, @)
...
\\
\arrow[rr, "", from=L, to=R, start anchor={[yshift=-0.2ex]}, mapsto] 
&  &  
\\
B &  & \fnclist(B)
\end{tikzcd}
where $\fnclist(f) = \map{f}$
\begin{tikzcd}
          \big[\, a_1, ... ,a_n \,\big]
        \arrow[d, ""]
        \\
...
We need to check that \(\map{\id{A}} = \id{\fnclist(A)}\) and \(\map{g} \comp \map{f} = \map{g \comp f}\).\\
By definition of the function \(\map\), it is easy to prove that \(\map{\id{A}} = \id{\fnclist(A)}\).
\begin{center}
    \begin{tikzcd}[ampersand replacement=\&]
...
List as a functor from the universal property of a free monoid:

\begin{center}
    \begin{tikzcd}[ampersand replacement=\&]
        A \arrow[r, "{[\text{--}]_A}"] \arrow [d, "f"] \&
        List(A) \arrow[d, "\exists! List(f)"] \& {=} \& (List(A), nil, @) \arrow[d, "{[\text{--}]_B\circ f}" , mapsto] 
           \\
        B \arrow[r, "{[\text{--}]_B}"] \&
        List(B) \& {=} \& (List(B), nil, @)
    \end{tikzcd} \\[3mm]
...
\begin{center}
    \begin{tikzcd}[ampersand replacement=\&]
        A \arrow[r, "{[\text{--}]_A}"] \arrow [d, "id_A"] \&
        List(A) \arrow[d, "\exists! List(id_A)"] \& {=} \& (List(A), nil, @) \arrow[d, "id_{\fnclist(A)} \text{ homomorphism}" , mapsto] 
           \\
        A \arrow[r, "{[\text{--}]_A}"] \&
        List(A) \& {=} \& (List(A), nil, @)
    \end{tikzcd} \\[3mm]
...
Associativity of $\fnclist$:

\begin{center}
\begin{tikzcd}
    A \arrow[d, "f"]  \arrow[dd, bend right=40, "g \comp f" swap] \arrow[r, "{[\text{--}]_A}"] & \fnclist(A) \arrow[d, dotted, "\fnclist(f)"] \arrow[dd, bend left=80, dotted, "\fnclist(g) \comp \fnclist(f)"] \\
    B \arrow[d, "g"] \arrow[r, "{[\text{--}]_B}"] & \fnclist(B) \arrow[d, dotted, "\fnclist(g)"] \\
    C \arrow[r, "{[\text{--}]_C}"] & \fnclist(C)
\end{tikzcd}
\end{center}

$\fnclist(g) \comp \fnclist(f)$ is an homomorphism such that $\fnclist(g) \comp \fnclist(f) \comp {[\text{--}]_A} = {[\text{--}]_C} \comp g \comp f$.\\
Therefore $\fnclist(g) \comp \fnclist(f) = \fnclist(g\comp f)$\\\\

Free commutative monoid construction (i.e. finite multiset) is a functor
\begin{center}
    \begin{tikzcd}
          S \arrow[r, "\eta^S"]  \arrow[rd, swap, "\forall\, f"]
        &
...
\begin{exercise}
Show that the free commutative monoid (\ie~finite multisets) construction is a functor. \label{ex:fcmonoidfunctor}
\end{exercise}

\begin{exercise}
Show that the free semilattice (commutative idempotent monoids --- \ie finite powersets) construction is a functor. \label{ex:fsemilatticefunctor}
\end{exercise}

\subsection{More examples: product and coproduct constructions}

The (locally small) category \(\catcat\) has as objects small categories, \(\mathbb{C}\), and as morphisms functors \(\mathbb{C} \morpharr{F} \mathbb{D}\). Identities are \(\Id{\mathbb{C}}\) and composition \(\mathbb{A} \morpharr{F} \mathbb{B} \morpharr{G} \mathbb{C}\) such that \((G \comp F)(A) \eqdef G(FA)\).
...
The product of two categories, $\smcat{A}$ and $\smcat{B}$, denoted $\smcat{A}\times\smcat{B}$ is a category whose objects are pairs $(A, B) \suchthat A \in \smcat{A}, B \in \smcat{B}$ and whose morphisms are arrows $(A', B') \morpharr{(f, g)} (A', B')$ such that $\id{(A, B)} = (\id{A}, \id{B})$ and $(f, g) \comp (f', g') = (f \comp f', g \comp g')$.
\end{definition}

\begin{center}
\begin{tikzcd}
    (A_1,B_1) \arrow[r, "{(f,g)}"] \arrow[rr, swap, bend right=80, "{(f',g')\circ(f,g) \eqdef (f'\circ f, g'\circ g)}"] & (A_2,B_2) \arrow[r, "{(f',g')}"] & (A_3,B_3)
\end{tikzcd}
\end{center}
...
\begin{tikzcd}
    ...
    {} &
    \stackrel{\mbox{$C$}}{\forall}
    \arrow[uu, dashed, "\exists!", "{(f,g)}" swap]
    \arrow[lu, "f"] \arrow[ru, swap, "g"] & {}
\end{tikzcd}
\end{center}

\begin{exercise}
	Show that the product construction is universal. \label{ex:produnivconstr}
    
\begin{center}
\begin{tikzcd}
    {} & {\smcat{A}\times\smcat{B}} \arrow[ld, swap, "\pi_1"] \arrow[rd, "\pi_2"] & {} \\
    \smcat{A} & {} & \smcat{B}
\end{tikzcd}
\end{center}
...
\begin{center}
\begin{tikzcd}
    A \arrow[d, "f"] & {(A, B)} \arrow[l, mapsto] \arrow[r, mapsto] \arrow[d, "{(f, g)}"] & B \arrow[d, "g"] \\
    A' & {(A', B')} & B'
\end{tikzcd}
\end{center}
\end{exercise}

The product and coproduct constructions are functors. Let \(\smcat{C}\) be a category with binary products for all \(A, B \in \Ob(\smcat{C})\). Let $\mathrm{Prod}(A, B)$ be the product of $A$ and $B$.

\begin{exercise}
	Show that $\mathrm{Prod}$ is functorial: \ie~show $\mathrm{Prod}(\id{A}, \id{B}) = \id{\mathrm{Prod}(A, B)}$ and $\mathrm{Prod}(f, g) \comp \mathrm{Prod}(f', g') = \mathrm{Prod}(f \comp f', g \comp g')$, so that the following diagram commutes. \label{ex:prodfunc}

\begin{center}
\begin{tikzcd}
    {} & \mathrm{Prod}(A, B) \arrow[ld, swap, "\pi_1"] \arrow[rd, "\pi_2"] \arrow[ddd, dashed, "\exists!" swap, "{\textrm{Prod}(f, g)}"] & {} \\
    A \arrow[d, swap, "f"] & {} & B  \arrow[d, "g"] \\
    A' & {} & B' \\
    {} & \mathrm{Prod}(A', B') \arrow[lu, "\pi_1'"] \arrow[ru, swap, "\pi_2'"] & {}
\end{tikzcd}
\end{center}
\end{exercise}
