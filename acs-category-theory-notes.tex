...
solution, any $U\ni u$ must admit a map to our solution $T\ni t$, as shown 
below.
\begin{center}
    \begin{tikzcd}
        S \arrow[r, "f"]
...
$S$, we want to freely construct a join semilatice as follows:
% Create an example environment?
\begin{center}
    \begin{tikzcd}[ampersand replacement=\&]
        S \arrow[r, "\iota"] \arrow[dr, swap, "\forall\ f_2"] \& 
        \tilde{S}
        \arrow[d, "\exists ! h"] 
        \& (\tilde{S},\lor_{\tilde{S}}) 
...
$x\in S$. Similarly, for any two, three, or more generally any finite numbers
of distinct elements of $S$, it must contain the join or union of all of them.
Since the operator is commutative, the order doesn't matter, and since it's
idempotent, only one copy of each element of $S$ can exist. The picture looks
something like this:
\begin{align*}
...
x \lor y \lor z &\qquad\leftrightarrow\qquad \setof {x,y,z} \subseteq S \\
 &\enspace\qquad\,\vdots \\
x_1 \lor \cdots \lor x_n &\qquad\leftrightarrow\qquad \setof{x_1, \ldots, x_n}
\subseteq S
\end{align*}
