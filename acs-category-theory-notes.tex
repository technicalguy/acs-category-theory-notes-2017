...
\newcommand{\scat}[1]{\mathcal{#1}} % small category
...
\begin{center}
    \begin{tikzcd}[ampersand replacement=\&]
        A \arrow[r, "{[\text{--}]_A}"] \arrow [d, "f"{left}] \&
        \fnclist(A)
        \arrow[d, "\fnclist(f)\eqdef {\big([\text{--}]_B\circ f\big){^\#}}"]
        \& \qquad \&
        (\fnclist(A), nil, @)
        \arrow[d, "\exists!\ {\big([\text{--}]_B\circ f\big)^{\#}}
          \text{ homomorphism}" ]
           \\
        B \arrow[r, "{[\text{--}]_B}"] \&
...
\begin{center}
    \begin{tikzcd}[ampersand replacement=\&]
        ...
        \&
        \fnclist(A)
        \arrow[d, "{\fnclist(\id{A}) \eqdef \big([\text{--}]_A\big)^\#}"]
        \& \&
        (\fnclist(A), nil, @)
        \arrow[d,
          "{\exists! \text{ homomorphism }
              \big([\text{--}]_A\big)^\# \,= \ \id{\fnclist(A)}}"]
        \\
        A \arrow[r, "{[\text{--}]_A}"] \&
...
$\big(\fnclist(g) \comp \fnclist(f) \big)\comp {[\text{--}]_A}
 = {[\text{--}]_C} \comp (g \comp f)$.  Therefore,
\[
 \fnclist(g) \comp \fnclist(f)
 \ = \  \big( {[\text{--}]_C} \comp (g \comp f) \big)^\#
...
\begin{center}
    \begin{tikzcd}
          S \arrow[r, "\setof{\text{--}}_S"]  \arrow[rd, swap, "\forall\, f"]
        &
        \mathcal M_f(S)
        \arrow[d, "f^\#"]
...
\section{Motivation for Natural Transformations}

Morphisms between functors with the same domain and codomain.


In the category $\catpreo$ with objects $(P, \le)$, the morphisms are monotone
functions $f, g: (P, \le_P) \to (Q, \le_Q)$. Morphisms can be pre-ordered as
well:

$$f \le g \eqdef \forall p \in P. f(p) \le g(p)$$.

In the category $\catcat$, the objects are locally small categories $\lscat C$.
The morphisms are functors $F, G : \lscat A \to \lscat B$.
Natural transformations $\varphi: F \Rightarrow G$ relate these functors.
...
A natural transformation $\varphi: F \Rightarrow G : \lscat A \to \lscat B$
is a family:
...
\begin{center}
    \begin{tikzcd}
        ...
        \\
        A \arrow [d, "\forall f"] &
        F A \arrow[r, "\varphi_A"] \arrow [d, "F f"] &
        G A \arrow[d, "G f"]
        \\
        A' &
        F A' \arrow[r, "\varphi_{A'}"] &
        G A'
    \end{tikzcd} \\[3mm]
\end{center}
...
Let $\lscat C$ be a locally small category. We define a functor
$\lscat C(\_, \_): \lscat C^{op} \times \lscat C \to \catset$ mapping objects
from $\lscat C^{op} \times \lscat C$ to the set of morphisms between the two
objects.
...
\begin{center}
  \begin{tikzcd}
    ...
    (A, B) \ar["{(f, g)}"{name=L}]{d}
    &
    \lscat C (A, B) \ar["{\lscat C(f, g) = \lambda h. g \circ h \circ f}"{name=R}]{d}
    &
    &
    A \ar["h"{name=T}]{rr}
    &
    ...
    \ar[from=T, to=2-7, mapsto, shorten <= 2pt, shorten >= 2pt]
  \end{tikzcd}
\end{center}

\subsection{List Functor}

The list functor \(\fnclist: \catset \to \catset\) is defined as:

\begin{center}
\begin{tikzcd}
A
\arrow[rr, "\fnclist", mapsto] & & B
\\
A
\arrow[dd, "f"{name=L, right}] & & List(A)
\arrow[dd, "\fnclist(f)"{name=R, left}]
...
The family of singleton constructions:
$$
{[\text{--}]} = \Big \{ {[\text{--}]}_A : A \to List(A) \Big \}_{A \in \catset}
$$

is a natural transformation from $\fncid{Set}$ to $\fnclist$:
...
\begin{center}
    \begin{tikzcd}[sep=6em]
      ...
      \catset
      \\
      \ar[Rightarrow, from=T, to=B, "{[\text{--}]}", shorten <= 2pt, shorten >= 2pt]
    \end{tikzcd}\\[3mm]
\end{center}

\subsection{List Flattening}

Let $flat_A : List(List(A)) \to List(A)$ be a function that flattens lists.

$$
flat = \Big \{ flat_A: List(List(A)) \to List(A) \Big\}_{A \in \catset}
$$

flat is a natural transformation:
...
\begin{center}
    \begin{tikzcd}[sep=6em]
      List(A)
        \ar["{[\text{--}]}_{List(A)}"]{r}
        \ar["id_{List(A)}"below]{rd}
      &
      List(List(A))
        \ar["(id_{List(A)})^\#\eqdef flat_A"]{d}
      \\
      &
      List(A)
    \end{tikzcd}\\[3mm]
...
\begin{exercise}
Show $flat_A$ is a natural transformation using the definition.
\end{exercise}

\subsection{Coproducts}

Let $\lscat C$ be a category with coproducts (sums).

\begin{center}
    \begin{tikzcd}[sep=6em]
      &
      A + B
        \ar[dotted, "{\exists![f+g]}"]{d}
      &
      \\
      A
        \ar["f"]{r}
...
We have a coproduct functor $\lscat C \times \lscat C \to \lscat C$ mapping
$A, B \mapsto A + B$:
...
\begin{center}
    \begin{tikzcd}[sep=3em]
      ...
      \\
      A'
        \ar["\iota_1^{A',B'}"above]{rd}
      &
      &
      B'
        \ar["\iota_2^{A',B'}"above]{ld}
      \\
      &
      A' + B'
      &
...
The family $\iota_1=\Big\{ \iota_1^{A, B}: A \to A + B \Big\}_{(a,b) \in \lscat C \times \lscat C}$
is a natural transformation.

\begin{center}
    \begin{tikzcd}[sep=6em]
      \lscat C \times \lscat C
        \ar[bend left=40, "\pi_1"{name=T, above}]{r}
        \ar[bend right=40, "+"{name=B, below}]{r}
      &
      \lscat C
      \\
      \ar[Rightarrow, from=T, to=B, "{\iota_1}", shorten <= 2pt, shorten >= 2pt]
    \end{tikzcd}\\[3mm]
\end{center}
...
\begin{exercise}
Show that $\iota_1$ is a natural transformation.
\end{exercise}

\subsection{TODO - Not sure what to call this}

For $A \in \Ob(\lscat C)$, let $\nabla_A:A + A$ be the unique map s.t.
...
\begin{center}
    \begin{tikzcd}
      ...
      &
      \lscat C
      \arrow[Rightarrow, from=1-3,to=B,"\nabla", shorten <= 2pt, shorten >= 2pt]
    \end{tikzcd}\\[3mm]
\end{center}

\begin{exercise}
Show that $\nabla = \Big\{ \nabla_A:A + A \to A \Big\}_{A \in \lscat C}$ is a natural transformation.
\end{exercise}

\section{Functor Categories}

\begin{definition}[Functor Category]
Given categories $\lscat X$ and $\lscat A$, the functor category
$\lscat A^\lscat X$ (or $[\lscat X, \lscat A]$) is the category
whose objects are functors from $\lscat X$ to $\lscat A$ and whose morphisms
$\lscat A^\lscat X(F, G)$ are natural transformations from $F$ to $G$.
The identity morphism $id_F : F \Rightarrow F : \lscat X \to \lscat A$ is
$id_F = \lbrace id_{FX}: FX \to FX \rbrace_{X \in \Ob(\lscat X)}$.
Composition is defined as follows:

$$
\gamma \circ \varphi \eqdef
\Big\{ (\gamma \circ \varphi)_X : F X \to H X \Big\}_{X \in \Ob(\lscat X)}
$$
...
\begin{center}
    \begin{tikzcd}[sep=6em]
      ...
      &
      \lscat A \\
      \ar[Rightarrow, from=F, to=G, "\varphi", shorten <= 2pt, shorten >= 2pt]
...
\begin{exercise}
If $\varphi$, $\gamma$ are natural transformations, prove that $\gamma \circ \varphi$
is also a natural transformation.
\end{exercise}

\begin{example}
Consider the terminal category and an arbitrary locally small category
$\lscat C$. The functor category $[\nelem 1, \lscat C]$ has:

\begin{itemize}
  \item Functors from $\nelem 1$ to $\lscat C$ as objects.

    \begin{center}
      \begin{tikzcd}
          * \arrow[out=150,in=210, loop, looseness=5, "id_*"left]
      \end{tikzcd}
      $\to \lscat C \cong \Ob(\lscat C)$
    \end{center}

  \item Natural transformations between functors as morphisms.
    $$F \xRightarrow[]{\varphi} G: \nelem 1 \to \lscat C$$

    $$\varphi = \Big\{ \varphi_* : F * \to G * \Big\}$$
\end{itemize}
...
\begin{example}
Let $\cat D$ be the category with two objects and two arrows:

\begin{center}
  \begin{tikzcd}
    *
      \arrow[out=150,in=210, loop, looseness=5, "id_*"left]
      \arrow[shift left=+1, "\alpha"above]{r}
      \arrow[shift left=-1, "\beta"below]{r}
    &
    \bullet \arrow[out=30,in=-30, loop, looseness=5, "id_\bullet"right]
  \end{tikzcd}
\end{center}

In the functor category $[\cat D, \catset]$, the objects are:

\begin{center}
  \begin{tikzcd}[sep=4em]
    E
      \arrow["\varphi_*"]{d}
      \arrow[shift left=+1, "s"above]{r}
      \arrow[shift left=-1, "t"below]{r}
    &
    N \arrow["\varphi_\bullet"]{d}
    &
    F
      \arrow[Rightarrow, "\varphi"]{d}
    \\
    E'
      \arrow[shift left=+1, "s'"above]{r}
      \arrow[shift left=-1, "t'"below]{r}
...
\begin{exercise}
Show that $[\cat D, \catset] \cong \catdirgraph$.
\end{exercise}
