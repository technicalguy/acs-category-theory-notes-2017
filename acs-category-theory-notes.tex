...
\begin{definition}
A functor \(F: \mathcal{A} \rightarrow \mathcal{B}\) specifies a map \(F: \Ob(\mathcal{A}) \rightarrow \Ob(\mathcal{B})\) such that \(\forall X, Y \in \Ob(\mathcal{A})\), there exists \(F_{X, Y}: \mathcal{A}(X, Y) \rightarrow \mathcal{B}(FX, FY)\) such that identities are preserved.
...
\\
\arrow[rr, "F_{XY}", from=L, to=R, mapsto] 
&  &  
\\
Y &  & FY
\end{tikzcd}
