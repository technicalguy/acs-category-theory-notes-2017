...
\DeclareMathOperator{\Ob}{\mathbf{Ob}}
\DeclareMathOperator{\Arr}{\mathbf{Arr}}
...
\newcommand{\nats}{\mathbb{N}}
\newcommand{\morpharr}[1]{\overset{#1}{\longrightarrow}}
...
\begin{definition} Categories (Internal)
  ...
  A$, also written $1_A$ or just $A$.
  
  Arrows come with a (well-typed) \emph{composition} operation $\circ$. Given $A
  \morpharr f B$ and $B \morpharr C$ we have $A \morpharr{g \circ f} C$.

  Composition and identity arrows must satisfy laws similar to the monoid laws:

  \begin{itemize}
  \item Composition is associative. Given any objects and arrows such that
  ...
  \item $\id X$ is an identity under composition. Given

    \[X \morpharr k Y\]

    we insist that

    \[k \circ \id X = k = \id X \circ k .\]
  \end{itemize}
\end{definition}

\begin{definition} Categories (Enriched)
  ...
  \item Composition is associative:
    ...
    \begin{tikzcd}
      ...
      \arrow[dl, "\circ_{B,C,D} \times \idfunc"] \arrow[dr, "\idfunc \times
      \circ_{A,B,C}"]
      & \\
  ...
  \end{center}
    ...
    NB Here, $\idfunc \eqdef x \mapsto x$ is the standard identity function.
    $\times$ is an operator that essentially runs two functions in parallel.
    Given
    ...
    we have
    \[f \times g : A_1 \times A_2 \rightarrow B_1 \times B_2\] with
    \[(f \times g)(x, y) \mapsto (f x, g y) .\]
    
  \item Identity is a proper left and right identity. First, notice that we can
    write

    \[\unitset \morpharr{\id A} \mathcal C(A, A)\]

    to express the idea that the identity morphism $\id A$ exists for every $A$.
    Then the right identity law is expressed as:
  ...
  \end{center}
    ...
    and the left identity law is expressed similarly:

    \begin{center}
    \begin{tikzcd}
      \unitset \times \mathcal C(A, B) \arrow[rr, "\id A \times \idfunc"]
      \arrow[dr, "\pi_2"] & &
      \mathcal C(A, A) \times \mathcal C(A, B) \arrow[dl, "\circ_{B,A,A}"] \\
      & \mathcal C(A, B)
    \end{tikzcd}
  \end{center}
\end{itemize}
...
composition is standard function composition. The set of morphisms can be
written as
...
$\catmon$ is a category whose objects are monoids $(M, 1, *)$ and whose
morphisms are monoid homomorphisms (\ie functions which preserve monoidal
structure).
...
Let $f : M_1 \rightarrow M_2$ and $g : M_2 \rightarrow M_3$ be monoid
homomorphisms (in the monoids given here). Then let $x, y \in M_1$ and consider

\begin{align*}
  (g \circ f)(x *_1 y)
  & = g (f (x *_1 y)) & \text{(definition of function composition)}\\
  & = g (f (x) *_2 f (y)) & \text{($f$ is a monoid homomorphism)}\\
  & = g(f(x)) *_3 g(f(y)) & \text{($g$ is a monoid homomorphism)}\\
  & = (g \circ f)(x) *_3 (g \circ f)(y)
...
we call $f$ monotone when for every $x, y \in P$ $x \le_P y \implies f(x) \le_Q
f(y)$.
...
\begin{framed}
  \begin{definition} Subcategory
    ...
    \item $\Arr(\mathcal S) \subseteq \Arr(\mathcal C)$ with $\id{}$ and
      composition in $\mathcal S$ as in $\mathcal C$.
    \end{itemize}
  \end{definition}
\end{framed}
...
Notice $\catfinset$ cut down objects but kept the morphisms, and $\catbij$ cuts
down morphisms but keeps the objects.
...
Identity is the identity relation which relates every element to itself:

\[\id A = \setof{(a, a) \;|\; a \in A}\]

and composition is relational composition:

\[R \circ_{A,B,C} S = \setof{(a, c) \;|\; \exists b \in B.\; (a, b) \in S \wedge
  (b, c) \in R} .\]
...
$\catpred$ is acategory whose objects are pairs $(S, P)$ of a set $S$ and a
predicate on $S$, $P \subseteq S$. Its morphisms are functions which respect the
predicates, \ie
...
Identity and composition are as for normal functions.
...
\begin{tikzcd}
    ...
    B
    \\
    {} & C \arrow[uu, dashed, "\exists!"]
    \arrow[lu, "f"] \arrow[ru, swap, "g"] & {}
\end{tikzcd}
\end{center}

Before, we stipulated that $P, A, B, C$ were all sets and $\alpha, \beta, f, g$
were all functions. However, we can relax those assertions and instead work in
an arbitrary category $\mathcal C$, so that $P, A, B, C \in \Ob(\mathcal C)$ and
the arrows are morphisms in $\mathcal C$ between their respective objects.

\begin{exercise}
  In $\catset$, we could take $P = A \times B$, $\alpha = \pi_1$ and $\beta =
  \pi_2$ as the final solution. What happens for the other categories described
  in this lecture?

  In particular, you should try $\catmon$, $\catpreo$ and $\catpred$. After
  that, try $\catrel$. NB in some categories, there will be no solution.
\end{exercise}
